...
device_dialog(GtkWindow *window, Statusbar *statusbar)
{
    ...
    if (widgets == NULL) {
        ...
        gtk_spin_button_set_value(GTK_SPIN_BUTTON(widgets->channel), 5.0);
        ...
        widgets->program_number = gtk_spin_button_new_with_range(1.0, 8.0, 1.0);
...
note_on_callback(GtkWidget *widget, gpointer data)
{
    ...
    program = gtk_combo_box_get_active(GTK_COMBO_BOX(widgets->program_bank)) * 8;
