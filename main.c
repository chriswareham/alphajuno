...
typedef struct {
    ...
    GtkWidget *filters_menu_item;
    GtkWidget *envelope_menu_item;
    ...
    EnvelopeDialog *envelope_dialog;
...
static void show_filters_dialog_callback(GtkWidget *, gpointer);
static void show_envelope_dialog_callback(GtkWidget *, gpointer);
...
main(int argc, char *argv[])
{
    ...
    gtk_window_set_title(GTK_WINDOW(widgets.window), "Alpha Juno");
    ...
    widgets.filters_dialog = new_filters_dialog(GTK_WINDOW(widgets.window));
    widgets.envelope_dialog = new_envelope_dialog(GTK_WINDOW(widgets.window));
...
create_edit_menu(MainWidgets *widgets)
{
    ...
    widgets->filters_menu_item = gtk_menu_item_new_with_mnemonic("_Filters...");
    g_signal_connect(G_OBJECT(widgets->filters_menu_item), "activate", G_CALLBACK(show_filters_dialog_callback), widgets);
    gtk_widget_set_sensitive(GTK_WIDGET(widgets->filters_menu_item), FALSE);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), widgets->filters_menu_item);

    widgets->envelope_menu_item = gtk_menu_item_new_with_mnemonic("_Envelope...");
    g_signal_connect(G_OBJECT(widgets->envelope_menu_item), "activate", G_CALLBACK(show_envelope_dialog_callback), widgets);
    gtk_widget_set_sensitive(GTK_WIDGET(widgets->envelope_menu_item), FALSE);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), widgets->envelope_menu_item);
...
tree_view_callback(GtkTreeSelection *selection, gpointer data)
{
    ...
    if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
        ...
        set_filters_parameters(widgets->filters_dialog, current_patch);
        set_envelope_parameters(widgets->envelope_dialog, current_patch);
        ...
        gtk_widget_set_sensitive(GTK_WIDGET(widgets->filters_menu_item), TRUE);
        gtk_widget_set_sensitive(GTK_WIDGET(widgets->envelope_menu_item), TRUE);
        ...
        clear_filters_parameters(widgets->filters_dialog);
        clear_envelope_parameters(widgets->envelope_dialog);
...
static void
show_filters_dialog_callback(GtkWidget *widget, gpointer data)
{
    MainWidgets *widgets = data;
    show_filters_dialog(widgets->filters_dialog);
}

static void
show_envelope_dialog_callback(GtkWidget *widget, gpointer data)
{
    MainWidgets *widgets = data;
    show_envelope_dialog(widgets->envelope_dialog);
}
...
show_callback(GtkWidget *widget, gpointer data)
{
    ...
    if (midi_get_device_count() && midi_open(midi_devices[0]->device)) {
