...
text(GMarkupParseContext *context, const gchar *text, gsize len, gpointer data, GError **error)
{
    ...
    case STATE_ALPHAJUNO:
        if (len) {
            g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, "content '%.*s' not allowed in element 'alphajuno'", (int)len, text);
        }
        break;
    case STATE_PARAM:
        if (len) {
